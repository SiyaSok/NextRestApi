...
export const PATCH = async (request: Request, context: { params: any }) => {
  ...
  try {
    ...
    return new NextResponse(
      JSON.stringify({ message: "updated Blog", category: updatedBlog }),
      { status: 200 }
    );
  } catch (error: unknown) {
    if (error instanceof Error) {
      return new NextResponse("Error in updated a blog: " + error.message, {
        status: 500,
      });
    }
...
export const DELETE = async (request: Request, context: { params: any }) => {
  ...
  try {
    ...
    if (error instanceof Error) {
      return new NextResponse(
        "Error in deleting the blogId: " + error.message,
        {
          status: 500,
        }
      );
